...
/** -----------------------------------------------------------------------------------------------------------------------
/** ----------------------------------------------------------------------------------------------------------------------- **/
	const XML_CONF_FILE = SYSLang::XML_CONF_FILE;
...
/** ----------------------------------------------------------------------------------------------------------------------- **/
	/** --------------------------------------------------------------------------------------------------- **
	/** --- Méthode de récupération des langues disponible dans le fichier de configuration des langues --- **
	/** --------------------------------------------------------------------------------------------------- **/
	public function get_avail_languages(){
		/** Si le fichier existe alors on récupère les langues enregistrées **/
		if(file_exists($this->_workspace.'/Languages/'.self::XML_CONF_FILE)){
			/** Ouvrir le fichier de configuration des langues **/
			$xmllanguages = new SimpleXMLElement(file_get_contents($this->_workspace.'/Languages/'.self::XML_CONF_FILE));
			
			$languages = array();
			$index = 0;
			
			foreach($xmllanguages as $lkey => $lvalue){
				$languages[strval($lvalue->attributes()->LANG)] = $index;
				$index++;
			}
		...
		/** On envois une erreur **/
			...
			$this->throw_error(sprintf('File "%s" not found in folder "%s". Check path first or use SYSLangCompilator::build_environnement().'), E_USER_ERROR);
...
/** ----------------------------------------------------------------------------------------------------------------------- **/
	...
	public function add_languages($lang=null){// fr-FR:Francais
		...
		if($lang === null){
			$this->throw_error('Missing argument 1 ($lang) for SYSLangCompilator::add_languages(). $lang represents the language and it\'s code as this pattern lang_code:lang_name.', E_USER_ERROR);
			return false;
		}
		...
		foreach(func_get_args() as $key => $value){
			...
			if(preg_match('#^[a-z]{2}-[A-Z]{2}:#', $value)){
				...
				if(!array_key_exists($code, $languages)){
					...
					$this->throw_error(sprintf('Language "%s" with lang code "%s" already exist in "%s".', $lang_name, $code, self::XML_CONF_FILE), E_USER_WARNING);
				}
			} 
			else {
				$this->throw_error(sprintf('Argument supplied "%s" not valide. It must be like this lang-LANG:Lang_name. SYSLangCompilator::add_languages skip this argument.', $value), E_USER_WARNING);
			}
		}
		
		/** Sauvegarder les modifications **/
		$this->save_xml($xmllanguages, $this->_workspace.'/Languages/'.self::XML_CONF_FILE);
	}
	
	/** ------------------------------------------------------------------------------ **
	/** --- Méthode de construction de l'environnement (pour première utilisation) --- **
	/** ------------------------------------------------------------------------------ **/
	public function build_environnement($first_pack=null){
		/** #1. Gestion des arguments - Erreur si aucune paramètre fournis **/
		if($first_pack === null){
			$this->throw_error('Missing argument 1 ($first_pack) for SYSLangCompilator::build_environnement(). $first_pack will be set as default language pack.', E_USER_ERROR);
		} else {
		...
		if(!file_exists($this->_workspace.'/Languages')){
			mkdir($this->_workspace.'/Languages');
		}
		...
		if(!file_exists($this->_workspace.'/Languages/'.self::XML_CONF_FILE)){
			...
			file_put_contents($this->_workspace.'/Languages/'.self::XML_CONF_FILE, $header, FILE_APPEND);
			file_put_contents($this->_workspace.'/Languages/'.self::XML_CONF_FILE, $openner, FILE_APPEND);
			...
			foreach($packs as $pkey => $pvalue){
				...
				file_put_contents($this->_workspace.'/Languages/'.self::XML_CONF_FILE, $record, FILE_APPEND);
			}
			
			file_put_contents($this->_workspace.'/Languages/'.self::XML_CONF_FILE, $closer, FILE_APPEND);
			...
			foreach($packs as $pkey => $pvalue){
				...
				if(!file_exists($this->_workspace.'/Languages/'.$pvalue[0])){
					mkdir($this->_workspace.'/Languages/'.$pvalue[0]);
				}
				
				/** Création d'un fichier de langue générique **/
				if(!file_exists($this->_workspace.'/Languages/'.$pvalue[0].'/generic.xml')){
					$header = '<?xml version="1.0" encoding="utf-8"?>'.PHP_EOL;
					...
					file_put_contents($this->_workspace.'/Languages/'.$pvalue[0].'/generic.xml', $header, FILE_APPEND);
					file_put_contents($this->_workspace.'/Languages/'.$pvalue[0].'/generic.xml', $openner, FILE_APPEND);
					file_put_contents($this->_workspace.'/Languages/'.$pvalue[0].'/generic.xml', $record, FILE_APPEND);
					file_put_contents($this->_workspace.'/Languages/'.$pvalue[0].'/generic.xml', $closer, FILE_APPEND);
				}
			}
		} else {
			$this->throw_error(sprintf('File "%s" already exist in "%s". SYSLangCompilator::build_environnement done nothing. Use SYSLangCompilator::add_languages($lang) instead', self::XML_CONF_FILE, $this->_workspace.'Languages/'), E_USER_WARNING);
		}
	}
	...
	public function compile($packages=null){
		...
		if($this->_ref_package !== null){
			...
			if(file_exists($this->_workspace.'/Languages/'.$this->_ref_package)){
				...
				/** #3. Obtenir le rapport MD5 de controle **/
					...
					if(!file_exists($this->_workspace.'/Languages/MD5.xml')){
						file_put_contents($this->_workspace.'/Languages/MD5.xml', '<?xml version="1.0" encoding="utf-8"?>'.PHP_EOL, FILE_APPEND);
						file_put_contents($this->_workspace.'/Languages/MD5.xml', '<packs>'.PHP_EOL, FILE_APPEND);
						file_put_contents($this->_workspace.'/Languages/MD5.xml', '</packs>'.PHP_EOL, FILE_APPEND);
					}
					
					// #3.2. Récupération du rapport MD5
					$this->_MD5_report = new SimpleXMLElement(file_get_contents($this->_workspace.'/Languages/MD5.xml'));
				...
				//$this->_MD5_report->asXML($this->_workspace.'/Languages/MD5.xml');
				$this->save_xml($this->_MD5_report, $this->_workspace.'/Languages/MD5.xml');
			} 
			else {
				$this->throw_error('The reference package "'.$this->_ref_package.'" does not exist.', E_USER_ERROR);
			}
		} 
		else {
			$this->throw_error('Can not compile because the reference package is not defined. Use SYSLangCompilator->use_package($package) first.', E_USER_ERROR);
		}
	}
		...
		if(count($lang) > 0){
			foreach($lang as $lkey => $lvalue){
				if(array_key_exists($lvalue, $languages)){
					...
					$this->throw_error(sprintf('The argument supplied "%s" not found in "%s". Argument is ignored.', $lvalue, self::XML_CONF_FILE), E_USER_WARNING);
				}
			}
		}
		else {
	...
	private function resources_builder($target_packages, $folder_path=null){
		...
		$full_path = $this->_workspace.'/Languages/'.$this->_ref_package;
					...
					if(preg_match('#\.xml$#i', $file)){
						...
						try {
							...
							foreach($resources as $res_key => $res_value){
								...
								/** Dans le cas contraire, il y à un soucis, on ne sais pas qu'elle ressource manipuler **/
								else {
									$this->throw_error('The key "'.$key.'" in "'.($folder_path.'/'.$file).'" is already used', E_USER_ERROR);
								}
								...
								if(array_key_exists($s_key, $this->_ref_packages_keys)){
									$this->throw_error('The key "'.$key.'" in "'.($folder_path.'/'.$file).'" is already used in file(s) '.implode(', ', $this->_ref_packages_keys[$s_key]).' [Ref package : '.$this->_ref_package.']', E_USER_WARNING);
							...
							foreach($target_packages as $pkey => $pvalue){
								$target_package_path = $this->_workspace.'/Languages/'.$pvalue.'/';
								...
								$this->save_xml($target_resources, $target_package_path.'/'.$file);
							}
						...
						/** Le fichier XML n'est pas valide et on ne peux pas le traiter **/
							...
							$this->throw_error('The XML file "'.$file.'" in "'.($folder_path.'/'.$file).'" can not be parsed : '.$e->getMessage(), E_USER_ERROR);
						}
					} 
					else {
						$this->throw_error('The file "'.$file.'" is not a XML file.', E_USER_WARNING);
					}
				}
			}
	...
	/** ------------------------------------------------------- **/
	public function save_xml($sxe, $file, $flag=null){
